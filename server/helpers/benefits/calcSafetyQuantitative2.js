...
const _calc = (
  ...
  console.log(util.inspect(ECmoj, false, null, true));
...
const calcSafetyQuantitative = (
  ...
  for(let way of ways) {
    ...
    console.log(`bikeExp: ${bikeExp}`);
    console.log(`pedExp: ${pedExp}`);
    console.log(`L: ${L}`);
  ...
  for(let c of COLUMNS) {
    for(let o of OUTCOMES) {
      for(let j of LOCATION_TYPES) {
        ECmoj_NEW[c].combined[o][j] = ECmoj_NEW[c].bicycling[o][j] + ECmoj_NEW[c].walking[o][j];
      }
    }
  }

  console.log(util.inspect(ECmoj_NEW, false, null, true));
